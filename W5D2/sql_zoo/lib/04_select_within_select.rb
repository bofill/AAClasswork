...
# ## HAVING / GROUP BY
# - `GROUP BY`: Used to aggregate data by a selected column
# - `HAVING` : Used in conjunction with `GROUP BY` in order to filter the results
# - You should always use an alias when aggregating data for better readability
#   - aliases give a temporary name to aggregated columns
# - aggregates:
#   - `avg`, `max`, `min`, etc.